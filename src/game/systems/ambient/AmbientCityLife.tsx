...
function sampleRectLoop(progress: number, halfX: number, halfZ: number, centerZ = -5): RectSample {
  ...
  if (distance < halfX * 2) {
    return { x: -halfX + distance, z: centerZ - halfZ, heading: Math.PI / 2 };
  }
  distance -= halfX * 2;

  if (distance < halfZ * 2) {
    return { x: halfX, z: centerZ - halfZ + distance, heading: 0 };
  }
  distance -= halfZ * 2;
...
function DroneVisual({ accent }: { accent: string }) {
  return (
    <group>
      <mesh castShadow>
        <boxGeometry args={[1.25, 0.28, 0.72]} />
        <meshStandardMaterial color="#20293a" metalness={0.8} roughness={0.25} />
      </mesh>
  ...
  return (
    <group>
      <mesh castShadow position={[0, 0.32, 0]}>
        <boxGeometry args={[1.05, 0.45, 2.05]} />
        <meshStandardMaterial color="#182233" roughness={0.32} metalness={0.62} />
      </mesh>
      <mesh position={[0, 0.58, -0.1]}>
        <boxGeometry args={[0.82, 0.28, 0.92]} />
        <meshStandardMaterial color="#334155" roughness={0.2} metalness={0.72} />
      </mesh>
      <mesh position={[0, 0.36, -1.04]}>
        <boxGeometry args={[0.58, 0.08, 0.04]} />
        <meshBasicMaterial color={accent} />
      </mesh>
      <mesh position={[-0.34, 0.28, 1.04]}>
        <boxGeometry args={[0.2, 0.09, 0.04]} />
...
export function AmbientCityLife() {
  ...
  useFrame(({ clock }) => {
    ...
    drones.forEach((drone, index) => {
      ...
      if (!group) return;
      const angle = (t * drone.speed + drone.phase) * Math.PI * 2;
      const x = Math.cos(angle) * drone.radiusX;
      const z = Math.sin(angle) * drone.radiusZ - 8;
      group.position.set(x, drone.y + Math.sin(t * 1.2 + index) * 0.45, z);
      group.rotation.y = -angle + Math.PI / 2;
      group.rotation.z = Math.sin(t * 0.8 + index) * 0.06;
    });

    vehicles.forEach((vehicle, index) => {
      const group = vehicleRefs.current[index];
      if (!group) return;
      const sample = sampleRectLoop(t * vehicle.speed + vehicle.phase, vehicle.halfX, vehicle.halfZ);
    ...
    pedestrians.forEach((pedestrian, index) => {
      ...
      if (!group) return;
      const raw = (t * pedestrian.speed + pedestrian.phase) % 2;
      ...
      group.position.set(x, 0.05 + Math.abs(Math.sin(t * 6 + index)) * 0.025, z);
  ...
  return (
    <group>
      {drones.slice(0, droneCount).map((drone, index) => (
        <group
          key={`ambient-drone-${index}`}
          ref={(node) => {
            droneRefs.current[index] = node;
          }}
        >
          <DroneVisual accent={drone.accent} />
        </group>
      ))}

      {vehicles.slice(0, vehicleCount).map((vehicle, index) => (
        <group
          key={`ambient-vehicle-${index}`}
          ref={(node) => {
            vehicleRefs.current[index] = node;
          }}
        >
          <AmbientVehicle accent={vehicle.accent} />
        </group>
      ))}

      {pedestrians.slice(0, pedestrianCount).map((pedestrian, index) => (
        <group
          key={`ambient-pedestrian-${index}`}
          ref={(node) => {
            pedestrianRefs.current[index] = node;
          }}
        >
          <Pedestrian jacket={pedestrian.jacket} />
        </group>
      ))}
    </group>
  );
